...
#### **🎯 搜索精度優化**
- **Wildcard 精確匹配**: 使用 `*關鍵詞*` 模式確保關鍵詞必須存在於商品名稱
- **多關鍵詞獨立查詢**: 每個關鍵詞都有獨立的 wildcard 查詢條件
- **平衡精確性與靈活性**: 既避免模糊匹配，又支持非連續短語
...
5. **🔎 混合搜索**: 使用 wildcard 精確匹配 + 語義增強分數提升
...
### v5.2.0 - Wildcard 精確搜索優化 (2025年1月) 🚀 [最新穩定版]
- 🎯 **Wildcard 精確匹配**：使用 `*關鍵詞*` 模式確保關鍵詞必須存在
- 🔍 **平衡精確性與靈活性**：既避免模糊匹配，又支持非連續短語
- ⚡ **修復搜索精度問題**：解決「綠色衣服」返回無關商品的問題
- 🧠 **保持智能預處理**：GPT-4o 同義詞處理和查詢優化
- 📊 **技術改進**：
  - `wildcard: { query: "*關鍵詞*", path: "name" }`
  - `allowAnalyzedField: true` 支持分析字段
  - 每個關鍵詞獨立 wildcard 查詢條件
