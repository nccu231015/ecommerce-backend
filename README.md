...
### 🔄 AI 混合搜索 (核心特色)
- **官方 RRF 混合搜索**: 使用 MongoDB 官方 `$rankFusion` 聚合階段實現 Reciprocal Rank Fusion
...
### 🔄 AI 混合搜索技術
- **MongoDB `$rankFusion`**: 官方 Reciprocal Rank Fusion 混合搜索實現
...
3. **官方 RRF 混合搜索** 執行：
   - **向量管道**: 理解語義相似性，搜索 `product_embedding` 字段
   - **全文管道**: 精確關鍵詞匹配，搜索 `name` 字段
   - **`$rankFusion`**: MongoDB 自動應用 Reciprocal Rank Fusion 算法融合結果
4. **⭐ LLM 智能推薦** 分析搜索結果，標記最符合用戶需求的商品
5. **前端** 顯示混合搜索結果，推薦商品帶有金色 "⭐ AI 推薦" 徽章
...
### v4.0.0 - MongoDB 官方 `$rankFusion` 實現 (2025年1月)
