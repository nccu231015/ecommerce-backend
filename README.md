...
```mermaid
graph TD
    A["用戶查看商品詳情"] --> B["獲取商品ID和詳情"]
    B --> C["向後端請求相關商品推薦<br/>GET /related-products/:productId"]
    C --> D{"商品有向量嵌入?"}
    D -->|是| E["向量相似度搜索<br/>MongoDB $vectorSearch"]
    D -->|否| F["類別和標籤匹配<br/>計算匹配分數"]
    E --> G{"找到足夠相關商品?"}
    G -->|是| H["返回向量相似商品<br/>recommendation_type: vector_similarity"]
    G -->|否| F
    F --> I{"找到足夠相關商品?"}
    I -->|是| J["返回類別標籤匹配商品<br/>recommendation_type: category_tag_match"]
    I -->|否| K["隨機精選商品<br/>recommendation_type: random"]
    H --> L["前端顯示相關商品<br/>帶推薦類型標記"]
    J --> L
    K --> L
...
### 🆕 相關商品推薦流程說明
1. **用戶查看商品**: 訪問商品詳情頁面
2. **後端請求**: 前端自動向 `/related-products/:productId` 發送請求
3. **多層推薦策略**:
   - **向量相似度**: 優先使用商品向量尋找語義相似商品
   - **類別標籤匹配**: 如向量搜索失敗，使用類別和標籤匹配
   - **隨機精選**: 最後後備方案，確保始終有推薦
4. **前端展示**: 在商品詳情頁底部顯示推薦商品，並標明推薦類型
...
### v5.5.0 - 相關商品推薦功能 (2025年1月) 🆕 [最新功能]
- ✨ **新功能**: 商品詳情頁面相關商品推薦
- 🧠 **多層推薦策略**: 向量相似度 → 類別標籤匹配 → 隨機精選
- 🔄 **自動降級機制**: 確保始終有推薦商品顯示
- 🎨 **視覺化標記**: 清晰標示推薦來源類型
- 🚀 **新增 API 端點**: `/related-products/:productId`
- 📱 **響應式設計**: 適配所有屏幕尺寸
